...
sites = ["MB01", "MB02", "MB05"]
sitesFOI = ["MB01"]
sitesFOI2 = ["MB05"]
fishsites = ["MB01", "MB02", "MB05"]
humpbacksites = ["MB01", "MB02", "MB03"]
...
df1 = pd.read_excel(metaFile, sheet_name="MB")
...
---
title: "Monterey Bay National Marine Sanctuary Soundscape Report"
date: "today"
output: 
  html_document: 
    fig_caption: true
...
```{python}
#| echo: false
display(HTML(standardTextDict["MBNMS"]))
```

### Summary of monitoring sites
```{python}
#| echo: false
display(HTML(gf.makeImage("MBNMSSiteMap.jpg", "MB Site Map", 850)))
...
display(HTML(gf.makeImage("MB01_FinalHist.png", "Seasonal Fish Choruses", altText= "Histograms of proportion of fish chorusing hours detected for five fish species by month in the Monterey Bay National Marine Sanctuary, comparing the most recent recording year (2022-2023) to all previous years of data (2019-2021).")))
display(HTML(standardTextDict["fish yearly fc"]))

display(HTML(standardTextDict["spacer"]))

display(HTML(gf.makeButtons(fishsites, "***_FinalRosePlotV2.png", "daily_fish_choruses", altText= "Rose plots (24 hour clock) of hourly proportion of fish chorusing detected for five fish species in the Monterey Bay National Marine Sanctuary.")))
display(HTML(standardTextDict["MB diel fish fc"]))
```

### When are the seasonal biologically-important times for whales?
```{python}
#| echo: false
display(HTML(standardTextDict["MARS-whales"]))
display(HTML(gf.makeImage("MARS_BlueFinCI.jpg", "Seasonal blue and fin", altText= "Time series scatter plot of the MARS whale call index (a proxy for abundance) for both blue and fin whales in the Monterey Bay National Marine Sanctuary. Both species have peak seasonal presence in the summer and fall months.")))
display(HTML(standardTextDict["blue/fin fc"]))

display(HTML(standardTextDict["humpback-whales"]))
display(HTML(gf.makeImage("plot_MBNMSHumpbackWhaleDetections.jpg", "time series humpback", 800, altText= "Scatter plot with GAM model curves of weekly count of humpback whale acoustic presence for song and non song calls in the Monterey Bay National Marine Sanctuary. Non-song calls peak during summer whereas song calls peak fall through spring with many more song calls than non-song calls.")))
display(HTML(standardTextDict["MB humpback curve fc"]))

display(HTML(standardTextDict["spacer"]))

display(HTML(gf.makeButtons(humpbacksites, "plot_***HumpbackWhaleDetections.jpeg", "seasonal_humpback", altText= "Boxplot of hours of humpback whale acoustic presence by week for song and non song calls in the Monterey Bay National Marine Sanctuary. Song calls are recorded significantly more, but usually absent during summer months."))) 
display(HTML(standardTextDict["humpback box fc"]))
```

```{python}
#| echo: false

#Additional resources for cetacean acoustic detections:
