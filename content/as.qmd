...
df1 = pd.read_excel(metaFile, sheet_name="AS")
...
---
title: "National Marine Sanctuary of American Samoa Soundscape Report"
date: "today"
output: 
  html_document: 
    fig_caption: true
...
```{python}
#| echo: false
display(HTML(standardTextDict["NMSAS"]))
```

### Summary of monitoring sites
```{python}
#| echo: false
display(HTML(gf.makeImage("ASNMSSiteMap.jpg", "AS Site Map", 850)))
...
display(HTML(gf.makeButtons(FOIsites, "plot_***-10000-16000_HMDstatus.jpg", "FOI", altText= "Time series plot of daily median sound levels (sound intensity measured by mean-square pressure in microPascal per Hertz) for a specific frequency band(s) of interest at this site, separated by year. Background color shading (blue, purple, gold) indicates low (<25th percentile), typical (25-75th percentile), and high sound levels (>75th percentile) across the entire dataset at this frequency band(s) for comparability with annual medians, marked with horizontal black dashed lines. Pie charts on the right hand side of the graphic show the proportion of daily median sound levels that fell within each category for each year, following the same color-coding and percentile bins.")))
...
display(HTML(standardTextDict["fish-AS"]))
display(HTML(gf.makeImage("raster_by_site_Damselfish_NMSAS.png", "rasterfishAS", 900, altText= "Heatmap of daytime damselfish detections at AS01 in Fagatele Bay from late March to May 2023, showing peak activity during mid-day to afternoon hours.")))

display(HTML(standardTextDict["damselfish fc"]))

display(HTML(standardTextDict["spacer"]))

display(HTML(standardTextDict["parrotfish-AS"]))
display(HTML(gf.makeImage("raster_by_site_Parrotfish_23.png", "rasterparrotfishAS", 900, altText= "Heatmap of parrotfish detections at AS01 in Fagatele Bay from late March to May 2023.")))

display(HTML(standardTextDict["parrotfish AS fc"]))
...
#display(HTML(standardTextDict["humpback-AS"]))
# display(HTML(gf.makeImage("AS_Humpback.png", "humpbackAS", 200, altText= "Humpback whale presence by month; whales were detected between August and November (June 2015–April 2016), with a data gap from April to June.")))
# 
# display(HTML(standardTextDict["AS humpback fc"]))

```

### When does ship noise contribute to the soundscape?
```{python}
#| echo: false

display(HTML(standardTextDict["vessel-AS"]))

display(HTML(gf.makeImage("raster_by_site_Vessel_NMSAS.png", "vesselAS", 900, altText= "Heatmap of vessel detections at AS01 in Fagatele Bay, showing higher activity during daylight hours from late March to May 2023.")))

display(HTML(standardTextDict["AS vessel fc"]))
```
