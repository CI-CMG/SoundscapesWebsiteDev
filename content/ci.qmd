...
sites = ["CI01","CI04", "CINMS_B", "NRS05"]# "SIOB"
sitesFOI = ["CI01"]
sitesFOIB = ["CI01","CI04", "NRS05"]# "SIOB"
sitesFOI2 = ["NRS05"]
sitesFOI3 = ["CINMS_B"]
fishsites = ["CI01","CI04"]
...
df1 = pd.read_excel(metaFile, sheet_name="CI")
...
---
title: "Channel Islands National Marine Sanctuary Soundscape Report"
date: "today"
output: 
  html_document: 
    fig_caption: true
...
```{python}
#| echo: false
display(HTML(standardTextDict["CINMS"]))
```

### Summary of monitoring sites
```{python}
#| echo: false
display(HTML(gf.makeImage("CINMSSiteMap.jpg", "CI Site Map", 850)))
...
display(HTML(gf.makePlotlyButtonsWithLabels(sites, sites, "plot_***_interactiveSeasonalSPL.html", "InteractiveSeasonalSPL")))
...
display(HTML(gf.makePlotlyButtonsWithLabels(sites, sites, "plot_***_interactiveAnnualSPL.html", "InteractiveAnnualSPL")))
...
display(HTML(gf.makeButtons(fishsites, "***_FinalRosePlotV2.jpg", "daily_fish_choruses", altText= "Rose plots (24 hour clock) of hourly proportion of fish chorusing detected for five fish species in the Channel Islands National Marine Sanctuary.")))
display(HTML(standardTextDict["CI diel fish fc"]))
```
...
display(HTML(standardTextDict["humpback-whales-CI"]))
display(HTML(gf.makeImage("plot_CINMSHumpbackWhaleDetections.jpg", "time series humpback", altText= "Scatter plot with GAM model curves of weekly count of humpback whale acoustic presence for song and non song calls in the Channel Islands National Marine Sanctuary. Non-song calls peak during summer whereas song calls peak fall through spring with many more song calls than non-song calls.")))
display(HTML(standardTextDict["CI humpback curve fc"]))

```

### When are we hearing vessel presence?

```{python}
#| echo: false

display(HTML(standardTextDict["vessel-CI"]))
display(HTML(gf.makeImage("Ship_Wind_Animation_SPL_Hourly.gif", "CI vessels gif", altText= "")))
display(HTML(standardTextDict["CI vessels fc"]))
